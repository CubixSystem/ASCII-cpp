...
class GrayscalePlotter : public Plotter {
  public:
    ...
    void DrawRectangle(int x1, int y1, int x2, int y2, double brightness,
                       bool fill = false);
    void DrawTriangle(int x1, int y1, int x2, int y2, int x3, int y3,
                      double brightness, bool fill = false);
    void DrawCircle(int center_x, int center_y, int radius, double brightness,
                    bool fill = false);
    ...
    [[nodiscard]] const std::vector<char> &GetPalette() const noexcept {
        return palette_;
    }
    [[nodiscard]] size_t GetPaletteSize() const noexcept {
  ...
  private:
    ...
    Convolve(const std::vector<std::vector<double>> &kernel) const;
    static std::vector<std::vector<double>>
    CreateGaussianKernel(int size, double sigma = 1.0);
