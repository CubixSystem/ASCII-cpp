...
char GrayscalePlotter::BrightnessToChar(double brightness) const {
    ...
    size_t index = static_cast<size_t>(brightness * (palette_.size() - 1));
    return palette_[index];
}

void GrayscalePlotter::DrawLine(const int x1, const int y1, const int x2,
                                const int y2, const double brightness) {
    Plotter::DrawLine(x1, y1, x2, y2, BrightnessToChar(brightness));
}

void GrayscalePlotter::DrawRectangle(const int x1, const int y1, const int x2,
                                     const int y2, const double brightness,
                                     const bool fill) {
    Plotter::DrawRectangle(x1, y1, x2, y2, BrightnessToChar(brightness), fill);
}

void GrayscalePlotter::DrawTriangle(const int x1, const int y1, const int x2,
                                    const int y2, const int x3, const int y3,
                                    const double brightness, const bool fill) {
    Plotter::DrawTriangle(x1, y1, x2, y2, x3, y3, BrightnessToChar(brightness),
                          fill);
}

void GrayscalePlotter::DrawCircle(const int center_x, const int center_y,
                                  const int radius, const double brightness,
                                  const bool fill) {
    Plotter::DrawCircle(center_x, center_y, radius,
                        BrightnessToChar(brightness), fill);
}

void GrayscalePlotter::FloodFill(const int x, const int y,
                                 const double brightness) {
    Plotter::FloodFill(x, y, BrightnessToChar(brightness));
}

void GrayscalePlotter::ScanlineFill(const int x, const int y,
                                    const double brightness) {
    Plotter::ScanlineFill(x, y, BrightnessToChar(brightness));
}

void GrayscalePlotter::DrawLinearGradient(const int x1, const int y1,
                                          const int x2, const int y2,
                                          const double start_brightness,
                                          const double end_brightness) {
    ...
    for (int y = y1; y <= y2; ++y) {
        for (int x = x1; x <= x2; ++x) {
            ...
            const double brightness =
                start_brightness + ratio * (end_brightness - start_brightness);
            GetCanvas().at(x, y) = BrightnessToChar(brightness);
        }
    }
}

void GrayscalePlotter::DrawRadialGradient(const int center_x,
    ...
    for (int y = center_y - radius; y <= center_y + radius; ++y) {
        for (int x = center_x - radius; x <= center_x + radius; ++x) {
            ...
            const double distance = std::sqrt(std::pow(x - center_x, 2) +
            ...
            const double brightness =
                center_brightness +
                ratio * (edge_brightness - center_brightness);
            GetCanvas().at(x, y) = BrightnessToChar(brightness);
        }
    }
}
...
double GrayscalePlotter::CalculateAverageBrightness() {
    ...
    for (size_t i = 0; i < palette_.size(); ++i) {
        const double brightness =
            static_cast<double>(i) / (palette_.size() - 1);
        char_to_brightness[palette_[i]] = brightness;
    }
    for (auto &pixel : GetCanvas()) {
        if (const auto it = char_to_brightness.find(pixel);
            it != char_to_brightness.end()) {
            total += it->second;
            count++;
        }
    }
...
std::vector<std::vector<double>> GrayscalePlotter::GetBrightnessMatrix() const {
    std::vector<std::vector<double>> matrix(
        GetCanvas().Height(), std::vector<double>(GetCanvas().Width()));
...
void GrayscalePlotter::AdjustBrightness(const double factor) {
    ...
    for (int y = 0; y < GetCanvas().Height(); ++y) {
        for (int x = 0; x < GetCanvas().Width(); ++x) {
            ...
            if (auto it = char_to_brightness.find(old_char);
                it != char_to_brightness.end()) {
                const double new_brightness =
                    std::clamp(it->second * factor, 0.0, 1.0);
...
void GrayscalePlotter::ApplyThreshold(const double threshold) {
    ...
    for (int y = 0; y < GetCanvas().Height(); ++y) {
        for (int x = 0; x < GetCanvas().Width(); ++x) {
            char old_char = GetCanvas().at(x, y);
            if (const auto it = char_to_brightness.find(old_char);
                it != char_to_brightness.end()) {
                const double new_brightness =
                    it->second >= threshold ? 1.0 : 0.0;
                GetCanvas().at(x, y) = BrightnessToChar(new_brightness);
            }
        }
    }
...
void GrayscalePlotter::SetPixelBrightness(const int x, const int y,
                                          const double brightness) {
    if (GetCanvas().InBounds(x, y)) {
        GetCanvas().at(x, y) = BrightnessToChar(brightness);
    }
...
GrayscalePlotter::CreateGaussianKernel(const int size, const double sigma) {
    if (size % 2 == 0) {
...
std::vector<std::vector<double>>
GrayscalePlotter::CreateBoxKernel(const int size) {
    std::vector<std::vector<double>> kernel(size, std::vector<double>(size));
...
std::vector<std::vector<double>> GrayscalePlotter::Convolve(
    ...
    std::vector<std::vector<double>> result(height,
            ...
            for (int ky = 0; ky < kernel_size; ++ky) {
                for (int kx = 0; kx < kernel_size; ++kx) {
                    ...
                    if (GetCanvas().InBounds(src_x, src_y)) {
                        const double pixel_brightness =
                            GetPixelBrightness(src_x, src_y);
...
void GrayscalePlotter::SetPalette(const std::vector<char> &new_palette) {
    if (!new_palette.empty()) {
        ...
        for (int y = 0; y < GetCanvas().Height(); ++y) {
            for (int x = 0; x < GetCanvas().Width(); ++x) {
                GetCanvas().at(x, y) =
                    BrightnessToChar(brightness_matrix[y][x]);
            }
        }
    }
}
