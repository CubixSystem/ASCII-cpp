...
class Plotter {
  public:
    ...
    void DrawTriangle(int x1, int y1, int x2, int y2, int x3, int y3,
                      char brush, bool fill = false);
    void DrawCircle(int center_x, int center_y, int radius, char brush,
                    bool fill = false);
    ...
    [[nodiscard]] std::map<char, int> ColorHistogram(int x1, int y1, int x2,
                                                     int y2) const;
    [[nodiscard]] static std::pair<char, char>
    MinMaxColors(const std::map<char, int> &color_weights);

    [[nodiscard]] std::unique_ptr<Canvas> ExtractRegion(int x1, int y1, int x2,
                                                        int y2) const;
    void PasteRegion(const Canvas &region, int x, int y);

    [[nodiscard]] const Canvas &GetCanvas() const noexcept {
        return *canvas_;
    }
  ...
  private:
    ...
    void DrawCircleBresenham(int center_x, int center_y, int radius,
                             char brush);
